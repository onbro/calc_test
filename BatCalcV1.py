...
HoursAnnual = st.sidebar.slider("Hours per Year", min_value=0, max_value=8760, value=8760)
...
data = pd.DataFrame(np.array([['Project Capacity (MW)', int(float(dfProjectDetail.iloc[1]['p_cap']))],
                              ['Turbine Manufacturer', dfProjectDetail.iloc[1]['t_manu']],
                              ['Number of Turbines', dfProjectDetail.iloc[1]['p_tnum']],
                              ['Turbine Model', dfProjectDetail.iloc[1]['t_model']],
                              ['Turbine Capacity (MW)', int(float(dfProjectDetail.iloc[1]['t_cap'])/1000)],
                              ['Turbine Rotor Diameter (m)', int(float(dfProjectDetail.iloc[1]['t_rd']))],
                              ['Project Year', int(float(dfProjectDetail.iloc[1]['p_year']))],
                              ['State', dfProjectDetail.iloc[1]['t_state']],
                              ['County', dfProjectDetail.iloc[1]['t_county']]]),
...
st.bar_chart(ProjFinancials.loc[str(InstallYear-1):]["Net Savings"])
